...
3. **Option C: Hybrid Architecture (Selected)** — Use LLM strictly for natural language policy parsing (converting human governance policies into structured rule definitions) and a pure Python Depth-First Search (DFS) engine over an in-memory `InternalGraph` normalized from DataHub for deterministic enforcement.
...
- **Latency & Reliability**: Graph traversal completes in < 40ms without network dependency on LLM provider endpoints.
