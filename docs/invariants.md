...
6. **Demo Reproducibility**: Target-leakage *behavior* is reproducible offline via a synthetic fixture (`demo/fixtures/`, including unit-test vocabulary such as `customer_status`). The judge-facing `churn_model_v2` → `discount_history` → `raw_billing.retention_discount` (`post_outcome`) scenario is seeded by `seed.py` against live GMS. Additional seeded models (`recommendation_model_v1`, `fraud_model_v3`) exist for approved / incomplete-lineage checks; they are not required for the primary judge path.
