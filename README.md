...
`pytest` covers the deterministic gate with network-independent tests plus three
optional live-GMS integration tests (collected only when DataHub is reachable).
Verified from a clean `python3.13 -m venv` install of this pinned
`requirements.txt`. They cover the fail-closed boundaries (a blocked verdict
cannot be rendered as approved, the gate cannot exit 0 on a non-live source),
URN handling, write-back plan/executor parity, and request-scoped write-back
deduplication.
